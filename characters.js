...
function makeCharacter(cfg) {
  ...
  if (cfg.variant === "king") {
    cuff = "#e8b74a";
    // 익선관: 날개는 둥근 원(귀로 읽힘)이 아니라 바깥으로 기운 얇은 매미날개 잎사귀 형태
    hat =
      '<g class="hat">' +
      '<path d="M 86 26 Q 70 24 64 10 Q 70 2 80 6 Q 88 14 86 26 Z" fill="#26221c"/>' +
      '<path d="M 114 26 Q 130 24 136 10 Q 130 2 120 6 Q 112 14 114 26 Z" fill="#26221c"/>' +
      '<path d="M 72 44 Q 72 18 100 18 Q 128 18 128 44 L 128 50 Q 100 42 72 50 Z" fill="#171412"/>' +
      '<path d="M 58 58 Q 58 34 100 34 Q 142 34 142 58 L 142 64 Q 100 54 58 64 Z" fill="#26221c"/>' +
      "</g>";
    // 곤룡포: 옥대 + 흉배(보)
    torsoDetail =
    ...
    hat =
      ...
      "</g>";
    // 수염: 턱선(머리 원)을 따라 귀 높이(y94)에서 시작하는 말굽형 — 얼굴 랜드마크에 정합
    beard =
      '<path d="M 56 94 Q 60 132 84 144 Q 100 152 116 144 Q 140 132 144 94 Q 146 120 138 138 Q 124 160 100 160 Q 76 160 62 138 Q 54 120 56 94 Z" fill="#3a332b"/>';
    // 두정갑: 금징 3단 + 요대 + 갑찰 자락
    var studs = "";
    for (var r = 0; r < 3; r++) {
      for (var i = 0; i < 6; i++) {
    ...
    // 갓: 챙(rx68)이 머리 폭(r58)을 넘고 + 크라운은 높은 원통형(총모자) — 낮으면 챙모자로 오독
    hat =
      '<g class="hat">' +
      '<ellipse cx="100" cy="56" rx="60" ry="7" fill="' + shade(skin, 0.15) + '"/>' +
      '<path d="M 56 60 Q 52 84 57 103 L 62 102 Q 58 84 61 60 Z" fill="#221c16"/>' +
      '<path d="M 144 60 Q 148 84 143 103 L 138 102 Q 142 84 139 60 Z" fill="#221c16"/>' +
      '<ellipse cx="100" cy="52" rx="68" ry="9" fill="#221c16"/>' +
      '<path d="M 78 52 L 74 8 Q 100 0 126 8 L 122 52 Q 100 45 78 52 Z" fill="#171412"/>' +
      '<ellipse cx="100" cy="8" rx="24" ry="5" fill="#221c16"/>' +
      "</g>";
    // 도포: 교차 깃(동정) + 옷고름
    torsoDetail =
