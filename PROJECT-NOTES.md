...
23. **`server_up.sh`는 플러그가 이미 ON이면 무력하다**(2026-07-15, EP.6 렌더). BIOS가 *AC 인가 시* 자동부팅이므로, 플러그가 켜진 상태에서 `plug on`은 전원 전환을 못 만들어 no-op. 실제로 서버가 "켜져 있으나 Tailscale offline(1d)" 상태로 멈춰 있었고 `server_up.sh`가 180s 타임아웃. **진단 순서**: ① `plug.py status`가 아니라 **DPS 19(cur_power ÷10 = W)**를 읽어 실제 가동 여부 판정 — 정지 2~5W / 가동 100W대(EP.6 실측 102.4W) ② Tailscale 피어 상태 확인. 전원 재기동은 **가동 중이면 하드 전원차단**이라 사용자 승인 필수. 재기동 후 0W → 99.6W(t+24s) → SSH(50s)로 복구됨. 서버 렌더 실측: 12코어에서 48s·1440프레임 **25초**.
