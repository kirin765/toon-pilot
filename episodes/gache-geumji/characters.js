...
function makeCharacter(cfg) {
  ...
  if (cfg.variant === "king") {
    ...
    hat =
      ...
      // 가체(얹은머리) — EP.31. 머리 둘레를 크게 두른 땋은 타래(중앙 단일 매스 — 좌우 쌍 돌기 금지 규칙 준수).
      // 땋은 결(교차 곡선 스트로크)이 시그니처 — 민무늬 검은 덩어리는 터번/모자로 읽힌다.
      // cfg.gache: true(1단) | "tall"(2단 — 높이 경쟁·압사 씬용)
      (cfg.gache
        ? // 두툼한 타래 덩어리(신윤복 트레머리 실루엣) — 머리카락과 같은 색으로 이어 붙여 '모자'로 안 읽히게.
          // 둥근 혹(타래 코일) 3~5개가 겹친 울퉁불퉁한 실루엣 + 땋은 결 스트로크가 시그니처.
          '<circle cx="52" cy="26" r="26" fill="' + hairC + '"/>' +
          '<circle cx="148" cy="26" r="26" fill="' + hairC + '"/>' +
          '<circle cx="72" cy="6" r="28" fill="' + hairC + '"/>' +
          '<circle cx="128" cy="6" r="28" fill="' + hairC + '"/>' +
          '<circle cx="100" cy="-2" r="30" fill="' + hairC + '"/>' +
          '<path d="M 28 38 Q 30 4 62 -10 Q 100 -26 138 -10 Q 170 4 172 38 Q 140 20 100 20 Q 60 20 28 38 Z" fill="' + hairC + '"/>' +
          // 타래 사이 고랑(어두운 선) — 코일이 감긴 덩어리임을 알린다
          '<g fill="none" stroke="#0f0b07" stroke-width="3" opacity="0.9">' +
          '<path d="M 62 -8 Q 70 8 66 24"/><path d="M 138 -8 Q 130 8 134 24"/>' +
          '<path d="M 100 -30 Q 96 -8 100 12"/></g>' +
          // 땋은 결 — 빗금 스트로크(밝은 톤)
          '<g fill="none" stroke="#4d4438" stroke-width="3" opacity="0.8">' +
          '<path d="M 36 24 Q 44 14 54 12"/><path d="M 56 4 Q 64 -4 74 -6"/>' +
          '<path d="M 84 -14 Q 94 -20 106 -18"/><path d="M 118 -12 Q 128 -10 136 -2"/>' +
          '<path d="M 146 10 Q 154 14 162 24"/><path d="M 46 34 Q 72 20 100 20 Q 128 20 154 34"/>' +
          "</g>" +
          (cfg.gache === "tall"
            ? '<circle cx="76" cy="-28" r="22" fill="' + hairC + '"/>' +
              '<circle cx="124" cy="-28" r="22" fill="' + hairC + '"/>' +
              '<circle cx="100" cy="-38" r="24" fill="' + hairC + '"/>' +
              '<path d="M 54 -18 Q 62 -50 100 -56 Q 138 -50 146 -18 Q 122 -32 100 -32 Q 78 -32 54 -18 Z" fill="' + hairC + '"/>' +
              '<g fill="none" stroke="#0f0b07" stroke-width="3" opacity="0.9">' +
              '<path d="M 84 -50 Q 82 -38 86 -28"/><path d="M 116 -50 Q 118 -38 114 -28"/></g>' +
              '<g fill="none" stroke="#4d4438" stroke-width="3" opacity="0.8">' +
              '<path d="M 66 -34 Q 76 -44 90 -46"/><path d="M 110 -46 Q 124 -44 134 -34"/></g>'
            : "") +
          // 금 떨잠 1점(측면) — 중앙 장식은 관(冠) 보석으로 읽혀 금지
          '<circle cx="60" cy="14" r="6" fill="#e8b74a"/><circle cx="60" cy="14" r="3" fill="#d94f37"/>'
        : "") +
      // 족두리 — EP.31. 정수리 중앙의 작은 검은 관(중앙 단일 매스). cfg.jokduri: true | "jewel"(구슬 장식)
      (cfg.jokduri
        ? '<path d="M 76 32 L 83 4 L 117 4 L 124 32 Q 100 24 76 32 Z" fill="#171412"/>' +
          '<path d="M 83 4 Q 100 -4 117 4 L 115 12 Q 100 5 85 12 Z" fill="#26221c"/>' +
          '<path d="M 78 28 Q 100 20 122 28 L 121 34 Q 100 26 79 34 Z" fill="#3a332c"/>' +
          (cfg.jokduri === "jewel"
            ? '<circle cx="100" cy="8" r="5" fill="#e8e2d4"/><circle cx="87" cy="18" r="4.5" fill="#e8e2d4"/>' +
              '<circle cx="113" cy="18" r="4.5" fill="#e8e2d4"/><circle cx="100" cy="22" r="4" fill="#d94f37"/>' +
              '<circle cx="79" cy="29" r="3.5" fill="#e8b74a"/><circle cx="121" cy="29" r="3.5" fill="#e8b74a"/>'
            : "")
        : "") +
      (cfg.royal
