...
- **새 소품은 그리기 전에 실물의 구성 요소를 3~6개로 목록화**하고(예: 거중기 = 사다리꼴 틀 + 상/하 도르래 + 줄 + 매단 돌 + 양측 물레), 요소 하나당 `<g data-obj>` 하나로 그린다. 목록에 없는 장식은 추가하지 않는다 — 요소 수가 아니라 시그니처 실루엣이 가독성을 만든다.
...
`npm run check` 통과 ≠ 화면이 멀쩡함(정적 인스펙터는 시각 문제를 못 잡는다). S6.5로 넘기기 전에 반드시:
```bash
npx hyperframes snapshot --at <씬별 대표시점,쉼표구분> --no-end --describe false -o <scratchpad>/snaps
```
대표 시점 = 각 씬에서 자막·스탬프·num-pop이 **모두 떠 있는 순간**(대개 start+dur−1.0 부근). 프레임마다 아래 5항목을 순서대로 대조, 하나라도 걸리면 수정 → 재캡처 → 전 항목 통과까지 반복:
...
이 체크리스트는 S8 블라인드 검수의 사전 필터다 — 여기서 걸러지면 렌더 재작업 왕복이 사라진다.
